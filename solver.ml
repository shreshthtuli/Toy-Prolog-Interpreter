...
| PListp of term * term | PList of term list;;
...
let rec mgu t u : substitution = 
  ...
  match (t, u) with
    (Const n, Const m) -> if n=m then [] else raise NOT_UNIFIABLE
  | (Var x, Const n) -> [(Var x, Const n)]
  | (Const n, Var x) -> [(Var x, Const n)]
  | (Var x, Var y) -> if (x = y) then [] else [(Var x, Var y)]
  | (PListp(x,y), Const n) -> raise NOT_UNIFIABLE
  | (Const n, PListp(x,y)) -> raise NOT_UNIFIABLE
  | (PList(x), Const n) -> raise NOT_UNIFIABLE
  | (Const(n), PList(x)) -> raise NOT_UNIFIABLE
  | (PListp(x,y), Var z) -> [(Var z, PListp(x,y))]
  | (Var z, PListp(x,y)) -> [(Var z, PListp(x,y))]
  | (PList(x), Var z) -> [(Var z, PList(x))]
  | (Var z, PList(x)) -> [(Var z, PList(x))]
  | (PListp(x,y), PListp(x',y')) -> let rec fold sigma t u = match (t,u) with
                  ([],[]) -> sigma
                | (t1::tr, u1::ur) -> fold (compose sigma (mgu (subst sigma t1) (subst sigma u1))) tr ur
                | _ -> raise Error in
          fold [] ([x;y]) ([x';y'])
  | (PList(t'), PList(u')) ->  let rec fold sigma t u = match (t,u) with
                  ([],[]) -> sigma
                | (t1::tr, u1::ur) -> fold (compose sigma (mgu (subst sigma t1) (subst sigma u1))) tr ur
                | _ -> raise Error in
          fold [] t' u'
...
[] -> ()
| (Sym(a,b))::l -> print_string a ; print_string " " ; print_list l

let print_var (v,a) = match (v,a) with
	(Var x, Sym(a, b)) -> print_string (x^" = "^a^"  ")
  |	(Var x, PList(l)) -> print_string(x^" = [");print_list(l);print_string("]   ");; 
...
let fact_edge_be =
Fact(Atom("edge", [Sym("b", []); Sym("e", [])]));;

let fact_edge_ac =
Fact(Atom("edge", [Sym("a", []); Sym("c", [])]));;

let fact_edge_ce =
Fact(Atom("edge", [Sym("c", []); Sym("e", [])]));;

let fact_edge_ic =
Fact(Atom("edge", [Sym("i", []); Sym("c", [])]));;
